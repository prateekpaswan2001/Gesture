...
import math
import pyautogui # for mouse controls

##########################
wCam, hCam = 1080, 720
frameR = 200  # Frame reduction so every 200th frame will be processed
smoothening = 8 # helps to reduce abrupt changes
#########################

cap = cv2.VideoCapture(0) # default camera, use 1 for additional camera
cap.set(3, wCam)
cap.set(4, hCam)
pTime = 0 # stores time frame of previous frame
plocX, plocY = 0, 0  # stores last tracked position
clocX, clocY = 0, 0  # stores current tracked position

prevThumb, prstThumb = 0, 0 
prev = 0
prevLength = 0

detector = htm.handDetector(maxHands=1) # to track only one hand
wScr, hScr = pyautogui.size() # to get the screen size
print(wScr, hScr)
...
while True:
    ...
    # Flip the axis horizontally since webcam frames are mirrored by default
    img = cv2.flip(img, 1)
